...
class DispatchEmailTasks extends Command
{
    protected $signature = 'tenant:dispatch-email-tasks {--date= : date to send for [format: "Y-m-d"]}';
    ...
    public function handle()
    {
        ...
        foreach ($tenants as $tenant) {
            ...
            $tasks = InvoiceEmailTask::where('status', 'pending')
                ->whereBetween('date_at', [$startDate, $endDate])
                ->get();

            foreach ($tasks as $task) {
                $task->update(['status' => 'processing']);
                SendInvoiceEmailJob::dispatch($task->id, $tenant->id);
            }
