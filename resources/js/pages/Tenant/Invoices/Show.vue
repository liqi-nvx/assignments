<template>
  <AuthenticatedLayout>
    <div class="max-w-5xl mx-auto space-y-6 p-6">
      ...
      <div class="bg-white p-6 rounded-xl shadow-sm border grid grid-cols-1 md:grid-cols-4 gap-6 text-xs text-gray-600">
        <div>
        ...
        <div>
          <p class="text-gray-400 uppercase font-semibold mb-1">Product Details</p>
          <p class="font-bold text-indigo-600 text-sm">{{ invoice.goods?.name }}</p>
          <p class="mt-1">Qty: <span class="font-mono font-bold text-gray-800">{{ invoice.quantity }}</span> &times; ${{ parseFloat(invoice.unit_price).toFixed(2) }}</p>
        </div>
        <div>
          <p class="text-gray-400 uppercase font-semibold mb-1">Timeline</p>
            ...
            <tbody class="divide-y divide-gray-100 text-gray-600">
              <tr v-for="pay in payments" :key="pay.id" class="hover:bg-slate-50/50 transition-colors">
                ...
                <td class="px-5 py-3 text-center">
                  <span v-if="pay.status === 1" class="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full font-bold text-[10px]">
                    SUCCESS
                  </span>
                  <span v-else class="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-bold text-[10px]">
                    FAILED
                  </span>
                </td>
              </tr>
              <tr v-if="payments.length === 0">
                <td colspan="4" class="px-5 py-8 text-center text-gray-400 italic">
                  No payment allocation history entries recorded for this invoice instance yet.
                </td>
              </tr>
            </tbody>
          </table>
